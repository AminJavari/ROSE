...
def embed(triplet_path, model_dir, node_count, learning_rate=0.5, L1_reg=0.00, L2_reg=0.0001, n_epochs=4000,
          batch_size=80, n_hidden=80, dimension=20):
    """
    Stochastic gradient descent optimization of SNE
    """
    f = open(triplet_path, 'rb')
    ...
    # compute number of mini-batches for training
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        filename = model_dir + str(epoch) + '.p'
...
if __name__ == '__main__':
    embed(triplet_path=settings.config[const.SLASHDOT_GRAPH_TRIPLET],
          model_dir=settings.config[const.SLASHDOT_EMBED_MODEL],
          node_count=settings.config[const.SLASHDOT_NODECOUNT])
