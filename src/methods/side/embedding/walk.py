...
def walk_to_disk(g, config):
  ...
  if g.directed:
    ...
    num_pairs_required = window_size * (walk_length - (window_size + 1) / 2)
  args_list = [(ppw, walk_length, window_size, num_pairs_required, subsample, file, iter_function, seed, __current_graph, __vertex2str)
...
def write_walks_to_disk(args):
  ...
  num_walks, walk_length, window_size, num_pairs_required, subsample, file_name, iter_function, seed, __current_graph, __vertex2str = args
  rand = np.random.RandomState(seed)
  g = __current_graph
  with open(file_name, 'w') as fout:
    for walk in iter_function(g, num_walks, walk_length, window_size, num_pairs_required, subsample, rand, __vertex2str):
      fout.write(u"{} &\n".format(u" ".join(walk)))
  return file_name


def walk_on_memory_default_iter(g, num_walks, walk_length, window_size, num_pairs_required, subsample, rand, __vertex2str):
  """
  ...
  for cnt in range(num_walks):
    ...
    for node in nodes:
      num_pairs = 0
      while num_pairs < num_pairs_required:
        walk = random_walk(g, walk_length, node, rand, subsample, __vertex2str)
        yield walk
...
def random_walk(g, walk_length, start, rand, subsample, __vertex2str):
  """
